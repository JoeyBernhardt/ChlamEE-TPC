...
dat.clustlog<-data.frame(log10(dat.clust[,-c(9)]), dat.clust[,c(9)])
colnames(dat.clustlog)[9]<- 'FWS.Fill.factor'

#Cluster
subflow<-flowPeaks(dat.clustlog[,c(1:5,7,9)], tol=0.25,h0=0.05, h=2)
#1:5,7,9 works well
...
scatter3Drgl(dat.clustlog$FWS.Range,dat.clustlog$FL.Red.Range,dat.clustlog$X2.FL.Red.Range,colvar = subflow$peaks.cluster, col=palette1[1:max(subflow$peaks.cluster)],xlab='FWS.Range', ylab='FL.Red.Range', zlab='X2.FL.Red.Range',ticktype="detailed")
...
scatter3Drgl(dat.good$FWS.Range,dat.good$FL.Red.Range,dat.good$X2.FL.Red.Range,colvar = dat.good$subflow.peaks.cluster, col=palette1[1:max(subflow$peaks.cluster)], xlab='FWS.Range', ylab='FL.Red.Range', zlab='X2.FL.Red.Range')
...
cleanflow<-flowPeaks(dat.good[,c(1:4,6,7,9)], tol=0.25,h0=0.05, h=2)
...
rf <- randomForest(factor(subflow.peaks.cluster) ~ FWS.Range + FL.Red.Range + X2.FL.Red.Range +
                     FL.Orange.Range + FL.Yellow.Range + FL.Red.Gradient + FWS.Fill.factor,
                   data=dat.clustlog2,importance=TRUE, ntree=1001, na.action=na.omit)

#Second round of cleaning
rf2 <- randomForest(factor(cleanflow.peaks.cluster) ~ FWS.Range + FL.Red.Range + X2.FL.Red.Range +
                      FL.Orange.Range + X2.FL.Red.Gradient + FL.Red.Gradient + FWS.Fill.factor,
                    data=dat.clustlog2,importance=TRUE, ntree=1001, na.action=na.omit)
...
classifydat<-function(x){
  ...
  dat2=dat[,list(log10(FWS.Range), log10(FL.Red.Range), log10(X2.FL.Red.Range),
                 log10(FL.Orange.Range), log10(FL.Yellow.Range),log10(FL.Red.Gradient),
                 FWS.Fill.factor)]

  # Rename columns
  setnames(dat2, 1:7, new=c('FWS.Range', 'FL.Red.Range', 'X2.FL.Red.Range', 'FL.Orange.Range',
                            'FL.Yellow.Range', 'FL.Red.Gradient', 'FWS.Fill.factor'))
  ...
  newdat = dat[,classification1:=as.numeric(rf.class)]
