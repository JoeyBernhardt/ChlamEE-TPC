...
fcs_files <- c(list.files("data-raw/20180926_122512-ANC4-SETUP", full.names = TRUE))
...
ggsave("figures/anc4-fc-b-wells-fl1-3-gated.png", height = 8, width =12)
...
all_fcs3 %>% 
	...
	facet_wrap( ~ well) + geom_vline(xintercept = 17000, color = "black") + 
	geom_hline(yintercept = 160000, color = "black")
ggsave("figures/anc4-fc-c-wells-fl1-3-gated.png", height = 8, width =12)
...
all_fcs3 %>% 
	...
	facet_wrap( ~ well) + geom_vline(xintercept = 17000, color = "black") + 
	geom_hline(yintercept = 160000, color = "black")
ggsave("figures/anc4-fc-d-wells-fl1-3-gated.png", height = 8, width =12)
...
all_fcs3 %>% 
	...
	facet_wrap( ~ well) + geom_vline(xintercept = 17000, color = "black") + 
	geom_hline(yintercept = 160000, color = "black")
ggsave("figures/anc4-fc-e-wells-fl1-3-gated.png", height = 8, width =12)
...
all_fcs3 %>% 
	...
	facet_wrap( ~ well) + geom_vline(xintercept = 17000, color = "black") + 
	geom_hline(yintercept = 160000, color = "black")
ggsave("figures/anc4-fc-f-wells-fl1-3-gated.png", height = 8, width =12)
...
RFU <- read_excel("data-raw/anc4-pilot-september2018/anc4-pilot-setup-2018-09-26-take2.xlsx", range = "B56:N64") %>% 
	rename(row = X__1) %>% 
...
ALL <- left_join(all_counts, RFU, by = "well") %>% 
	mutate(cells_ml = n*40) %>% 
	mutate(Treatment = ifelse(is.na(Treatment), Population, Treatment)) %>% 
	mutate(rfu_ml = RFU*10)


ALL %>% 
	ggplot(aes(x = cells_ml, y = rfu_ml, color = Treatment)) + geom_point(size = 3) +
	geom_abline(slope = 1, intercept = 0) + ylab("RFU per ml") + xlab("Cells per ml")
ggsave("figures/anc4-setup-plate.pdf", width = 6, height = 4)



ALL %>% 
	ggplot(aes(x = Population, y = RFU, color = Treatment)) + geom_point(alpha = 0.5, size = 2)
ggsave("figures/anc4-setup-RFU.pdf", width = 6, height = 4)
...
means <- ALL %>% 
	...
	mutate(twelve = mean_RFU/8) %>% 
	mutate(dilution = 1/twelve) %>% 
	mutate(volume_needed = dilution*30) %>% 
...
### ok let's aim for a starting RFU of about 12
### this means: 
