...
RFU_files <- c(list.files("data-raw/chlamee-acute-dilution-RFU", full.names = TRUE))
RFU_files <- RFU_files[grepl(".xlsx", RFU_files)]
RFU_files <- RFU_files[grepl("plate1", RFU_files)]
...
high_conc1 <- dilutions %>% 
	...
	mutate(volume_needed = dilution2*50) %>% 
	mutate(combo_needed = 50-volume_needed) %>% 
	...
	mutate(combo_needed_divided_x5 = round(digits = 3, combo_needed/5)) %>% 
	mutate(combo_needed_divided_x2 = round(digits = 3, combo_needed/2)) %>% 
	select(population, volume_needed, combo_needed_divided_x2, combo_needed_divided_x5, combo_needed)
...
RFU_files <- c(list.files("data-raw/chlamee-acute-dilution-RFU", full.names = TRUE))
RFU_files <- RFU_files[grepl(".xlsx", RFU_files)]
RFU_files <- RFU_files[grepl("plate2", RFU_files)]
RFU_files
...
high_conc2 <- dilutions2 %>% 
	...
	mutate(combo_needed_divided_x5 = round(digits = 3, combo_needed/5)) %>% 
	mutate(combo_needed_divided_x2 = round(digits = 3, combo_needed/2)) %>% 
	select(population, volume_needed, combo_needed_divided_x5, combo_needed_divided_x3, combo_needed)

high_conc1 <- high_conc1 %>% 
	mutate(population = as.character(population))
all_conc <- bind_rows(high_conc1, high_conc2) %>% 
	select(population, volume_needed, combo_needed_divided_x5, combo_needed)

write_csv(all_conc, "data-processed/chlamee-acute-dilutions.csv")
