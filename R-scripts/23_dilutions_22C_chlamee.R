...
RFU_files <- c(list.files("data-raw/chlamee-acclimated-dilution-RFU/22C", full.names = TRUE))
...
high_conc2 <- dilutions2 %>% 
	# filter(dilution2 > 0.15) %>% 
	mutate(volume_needed = dilution2*6) %>% ### here the volume corresponds to the volume needed
	mutate(combo_needed = 6-volume_needed) %>% 
	mutate(volume_needed = round(digits = 3, volume_needed)) %>% 
	mutate(combo_needed = round(digits = 3, combo_needed)) %>% 
	mutate(over_5 = ifelse(volume_needed > 5, "yes", "no")) %>% 
...
write_csv(all_conc, "data-processed/chlamee-dilutions-22C.csv")
