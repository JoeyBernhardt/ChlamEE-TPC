...
plate_layout1 <- read_excel("data-general/chlamee-dilution-serial.xlsx") %>% 
...
RFU_files <- c(list.files("data-raw/chlamee-acclimated-dilution-RFU/serial-test", full.names = TRUE))
...
all_rfu <- left_join(all_temp_RFU, plate_layout1, by = "well")  %>% 
	dplyr::filter(!grepl("setup-", file_name))
...
dilutions <- all_rfu %>% 
	...
	mutate(dilution_full2 = mean_rfu/15) %>% 
	mutate(dilution3 = 1/dilution_full2) %>% 
	mutate(dilution_full3 = mean_rfu/25) %>% 
	mutate(dilution4 = 1/dilution_full3)

high_conc1 <- dilutions %>% 
	# filter(dilution2 > 0.15) %>% 
	mutate(volume_needed1 = round(digits = 3, dilution2*6)) %>% 
	mutate(combo_needed1 = round(digits = 3, 6-volume_needed1)) %>% 
	mutate(volume_needed2 = round(digits = 3, dilution3*6)) %>% 
	mutate(combo_needed2 = round(digits = 3, 6-volume_needed2)) %>% 
	select(population, volume_needed1, combo_needed1, volume_needed2, combo_needed2) %>% View 
