...
df_raw <- read_csv("/Users/joeybernhardt/Documents/too-big-for-github/dilution-test-high-density-particles.csv")
df <- df_raw %>% 
	dplyr::filter(ssc_a > 0, fsc_a > 0, fl1_a > 0, fl2_a >0, fl3_a>0, fl4_a>0) %>% 
	select(well, ssc_a, fsc_a, fl1_a, fl2_a, fl3_a, fl4_a) %>% 
...
#Cluster
subflow <- flowPeaks(dat.clust[,c(1:6)], tol=0.25, h0 = 0.05, h=2)


# examine details if interested. The 'weight' column indicates the proportion of the data belonging
# to that cluster
summary(subflow)
...
df_class <- left_join(df, dat.clust2, by = c("fl1_a", "fl2_a", "fl3_a", "ssc_a", "fsc_a", "fl4_a"))

### here we want cluster class 1, this is the algae

write_csv(df_class, "data-processed/anc4-classified.csv")
write_csv(df_class,"/Users/joeybernhardt/Documents/too-big-for-github/dilution-test-high-density-particles-classified.csv")

cell_counts <- df_class %>% 
group_by(well, subflow.peaks.cluster) %>% 
	tally() %>% 
	dplyr::filter(subflow.peaks.cluster == "1")

write_csv(cell_counts, "data-processed/cell_counts_flow_peaks_high_density_dilutions.csv")

#Examine any plots of interest
scatter3Drgl(dat.clust$fl1_a,dat.clust$fl2_a,dat.clust$fl3_a,dat.clust$fl4_a, colvar = subflow$peaks.cluster, col=palette1[1:max(subflow$peaks.cluster)],xlab='fl1', ylab='fl2', zlab='fl3',ticktype="detailed")


plot(dat.clust$fl4_a,dat.clust$fl3_a)

#Re-examine plot without the putative junk clusters (this may reveal smaller ones that were not
#visible earlier)
junk.clusterids<-c(2,3, 4, 5)
dat.clustlog2<-data.frame(dat.clust,subflow$peaks.cluster)
